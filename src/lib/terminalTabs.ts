 ...
 * 버전 판정만 예외로 [resolveZellijIdTargetingSupport]가 실행기를 인자로 받아 답을 캐시한다.
 */
...
const zellijIdTargetingSupportByHost = new Map<string, boolean>();

/**
 * 이 호스트의 zellij가 탭·pane을 id로 지정하는 명령을 받는지 판정한다.
 *
 * 이름이 축 하나를 고르지 않고 버전 경계를 가리키는 이유는, 부르는 곳이 탭 축(`hasZellijTabIdSupport`)과
 * pane 축(`hasZellijPaneIdSupport`) 둘 다이기 때문이다. 한쪽 축의 이름을 쓰면 다른 쪽 호출이 거꾸로 읽힌다.
 * 서비스마다 캐시를 두면 같은 호스트에 `--version`을 두 번 묻고 판정 규칙도 두 곳으로 갈라지므로,
 * 규칙과 캐시를 여기 하나로 둔다.
 * 명령 실행을 인자로 받는 이유는, 호출자마다 타임아웃과 원격 경로가 달라 이 파일이 직접 실행할 수 없기 때문이다.
 */
export async function resolveZellijIdTargetingSupport(
  sshHost: string | null,
  run: (command: string) => Promise<string>,
): Promise<boolean> {
  const hostKey = sshHost ?? "local";
  const cachedSupport = zellijIdTargetingSupportByHost.get(hostKey);
  ...
  zellijIdTargetingSupportByHost.set(hostKey, isSupported);
  return isSupported;
}
