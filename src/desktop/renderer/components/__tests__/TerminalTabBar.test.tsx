...
describe("TerminalTabBar", () => {
  ...
  it("앞 5개 탭에는 이동 단축키를 표시한다", () => {
    ...
    const [firstTab, secondTab] = screen.getAllByRole("tab");
    expect(firstTab.textContent).toContain("Ctrl+Shift+1");
    expect(secondTab.textContent).toContain("Ctrl+Shift+2");
  });
  ...
  it("6번째 탭부터는 대응하는 단축키가 없어 힌트를 붙이지 않는다", () => {
    ...
    expect(tabs[4].textContent).toContain("Ctrl+Shift+5");
    expect(tabs[5].textContent).not.toContain("Ctrl+Shift+");
  });
