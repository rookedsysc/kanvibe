...
> KanVibe는 이제 내장 SQLite 데이터베이스를 사용합니다. Docker는 더 이상 필요하지 않습니다.
...
의존성 체크(i18n 설치 프롬프트 포함), 패키지 설치, 내장 SQLite 데이터베이스 준비, 빌드, 서버 실행까지 모두 처리됩니다.
