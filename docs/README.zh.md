...
用于实时管理 AI 编程代理（Claude Code、Gemini CLI、Codex CLI 等）任务的 Tauri 终端看板。
在桌面应用中直接监控 tmux/zellij 会话，同时通过拖放看板追踪任务进度。
...
KanVibe 现在作为 Tauri 桌面应用运行。所需依赖如下：
...
AI 代理 Hooks 触发的任务状态变更会发送**桌面通知**，显示项目名称、分支名称和新状态。**点击通知可直接跳转到对应任务详情页面**。
...
| `/api/hooks/status` | POST | 通过 `branchName` + `projectName` 更新任务状态；如果目标不存在，则只发送桌面通知并保持 `404` 响应，不会修改状态 |

### GitHub 风格 Diff 视图

在桌面应用中使用 GitHub 风格的 diff 查看器直接审查代码变更。点击任务详情页面上的 **Diff** 徽章，即可查看与 base 分支相比的所有修改文件。
...
| 类别 | 技术 |
|------|------|
| 前端/后端 | Tauri v2 + React 19 + TypeScript |
| 数据库 | SQLite + TypeORM |
| 样式 | Tailwind CSS v4 |
| 终端 | xterm.js + local session bridge + node-pty |
| SSH | ssh2 (Node.js) |
| 拖放 | @hello-pangea/dnd |
| 国际化 | 桌面静态文案 |
| 桌面壳 | Tauri v2 |
