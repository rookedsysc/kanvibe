...
在主看板顶部点击位于项目筛选和 **+ 新任务** 之间的**放大镜目录扫描按钮**，打开项目注册对话框。在对话框中选择本地或远程 git 仓库路径并点击 **扫描并注册** 后，KanVibe 会注册仓库、检测现有 worktree 分支，并安装支持的代理 hook。

如果要停止管理某个项目，请在注册对话框底部的已注册项目列表中删除它。此操作只会移除 SQLite 中的项目记录及关联的 KanVibe 任务记录；磁盘上的 git 分支、worktree 和文件都会保留。
