...
contextBridge.exposeInMainWorld("kanvibeDesktop", {
  ...
  openTerminal(taskId, tabId, cols, rows) {
    return ipcRenderer.invoke("kanvibe:terminal-open", taskId, tabId, cols, rows);
  },
  ...
  onTerminalTabShortcut(listener) {
    const handler = (_event, payload) => listener(payload);
    ipcRenderer.on("kanvibe:terminal-tab-shortcut", handler);
    return () => {
      ipcRenderer.removeListener("kanvibe:terminal-tab-shortcut", handler);
    };
  },
  ...
  onTaskDetailUsageShortcut(listener) {
    const handler = () => listener();
    ipcRenderer.on("kanvibe:task-detail-usage-shortcut", handler);
    return () => {
