...
export function registerDiffHandlers(): void {
   ...
   */
  ipcMain.handle(
    "diff:getGitDiffFiles",
    async (_event, taskId: string): Promise<DiffFile[]> => {
      try {
        ...
        const [nameStatusResult, numstatResult, workingTreeResult] =
          await Promise.all([
            execAsync(
              `git diff ${baseBranch}...${branchName} --name-status`,
              { cwd: worktreePath }
            ).catch(() => emptyResult),
            execAsync(
  ...
  ipcMain.handle(
    "diff:getOriginalFileContent",
    async (_event, taskId: string, filePath: string): Promise<string> => {
      try {
