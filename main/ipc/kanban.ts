...
export function registerKanbanHandlers(): void {
  ...
  ipcMain.handle("kanban:getTasksByStatus", async () => {
    ...
    return { tasks: grouped, doneTotal, doneLimit: DONE_PAGE_SIZE };
  });

  /** Done 태스크를 추가 로드한다 */
  ipcMain.handle(
    "kanban:getMoreDoneTasks",
    async (_event, offset: number, limit: number = DONE_PAGE_SIZE) => {
      const repo = getTaskRepository();
  ...
  /** 같은 프로젝트 내에서 branchName이 일치하는 태스크 ID를 조회한다 */
  ipcMain.handle(
    "kanban:getTaskIdByProjectAndBranch",
  ...
  ipcMain.handle(
    ...
    async (_event, taskId: string, newStatus: TaskStatus) => {
      const repo = getTaskRepository();
  ...
  ipcMain.handle(
    "kanban:updateProjectColor",
    async (_event, projectId: string, color: string) => {
  ...
  ipcMain.handle("kanban:deleteTask", async (_event, taskId: string) => {
    const repo = getTaskRepository();
    const task = await repo.findOneBy({ id: taskId });
   ...
   */
  ipcMain.handle(
    "kanban:connectTerminalSession",
    async (_event, taskId: string, sessionType: SessionType) => {
      const repo = getTaskRepository();
      const task = await repo.findOneBy({ id: taskId });
  ...
  ipcMain.handle(
    "kanban:reorderTasks",
    async (_event, _status: TaskStatus, orderedIds: string[]) => {
      const repo = getTaskRepository();
