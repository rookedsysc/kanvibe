...
/// Generate an SGR motion report for mouse-aware terminal applications.
pub fn mouse_motion_report(
    ...
    let report_drag = mode.contains(TermMode::MOUSE_DRAG) && pressed_button.is_some();
    if !report_motion && !report_drag {
        return None;
    }
...
pub fn scroll_report(
    ...
    if mode.intersects(TermMode::MOUSE_REPORT_CLICK | TermMode::MOUSE_MOTION | TermMode::MOUSE_DRAG)
    {
        ...
        // Mouse wheel events are always "pressed" (M), never released. Emit one
        // report per line, as terminal applications use event count for speed.
        let event = format!("\x1b[<{};{};{}M", button_value, col, row);
        let count = delta.unsigned_abs().clamp(1, 10) as usize;
        let mut reports = Vec::with_capacity(event.len() * count);
        for _ in 0..count {
            reports.extend_from_slice(event.as_bytes());
        }
        return Some(reports);
    }
...
/// Convert pixel scrolling into whole terminal rows while retaining trackpad
/// fractions between events.
pub fn accumulate_scroll_lines(
    pixel_delta: Pixels,
    cell_height: Pixels,
    remainder: &mut f32,
) -> i32 {
    let pixel_delta: f32 = pixel_delta.into();
    let cell_height: f32 = cell_height.into();
    if cell_height <= 0.0 {
        return 0;
    }

    *remainder += pixel_delta / cell_height;
    let lines = remainder.trunc().clamp(-10.0, 10.0) as i32;
...
mod tests {
    ...
    #[test]
    fn accumulates_precise_trackpad_scroll_without_reversing_direction() {
        let mut remainder = 0.0;
        assert_eq!(
            accumulate_scroll_lines(px(8.0), px(20.0), &mut remainder),
            0
        );
        assert_eq!(
            accumulate_scroll_lines(px(12.0), px(20.0), &mut remainder),
            1
        );
        assert_eq!(
            accumulate_scroll_lines(px(-40.0), px(20.0), &mut remainder),
            -2
        );
    }

    #[test]
    fn mouse_scroll_emits_one_report_per_row() {
        let point = AlacPoint::new(Line(2), Column(3));
        let reports = scroll_report(3, point, 0, TermMode::MOUSE_REPORT_CLICK).unwrap();
        assert_eq!(reports, b"\x1b[<64;4;3M\x1b[<64;4;3M\x1b[<64;4;3M");
    }

    #[test]
    fn motion_reports_distinguish_hover_and_drag() {
        let point = AlacPoint::new(Line(1), Column(2));
        assert_eq!(
            mouse_motion_report(None, point, 0, TermMode::MOUSE_MOTION).unwrap(),
            b"\x1b[<35;3;2M"
        );
        assert_eq!(
            mouse_motion_report(Some(MouseButton::Left), point, 0, TermMode::MOUSE_DRAG).unwrap(),
            b"\x1b[<32;3;2M"
        );
        assert!(mouse_motion_report(None, point, 0, TermMode::MOUSE_DRAG).is_none());
    }
