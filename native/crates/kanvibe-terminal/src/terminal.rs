...
impl TerminalState {
    ...
    /// Create terminal state with an explicit scrollback history limit.
...
mod tests {
    ...
    #[test]
    fn explicit_scrollback_limit_is_applied() {
        let (tx, _rx) = channel();
        let event_proxy = GpuiEventProxy::new(tx);
        let mut terminal = TerminalState::with_scrollback(10, 3, 4, event_proxy);
        for _ in 0..20 {
            terminal.process_bytes(b"line\r\n");
        }

        terminal.with_term(|term| {
            assert_eq!(term.grid().total_lines() - term.grid().screen_lines(), 4);
        });
    }
