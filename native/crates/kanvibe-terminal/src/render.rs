...
use gpui::{
    ...
    SharedString, Size, TextRun, UnderlineStyle, Window, px, quad, rgb, transparent_black,
};
...
impl TerminalRenderer {
    ...
    pub fn paint(
        ...
        // Iterate over viewport lines, translating them into scrollback-aware
            ...
            if let Some(selection) = selection {
                for col_idx in 0..num_cols {
                    ...
                    let selection_bounds = Bounds {
                        origin: Point {
                            x: origin.x + self.cell_width * col_idx as f32,
        ...
        // The cursor lives on the live screen, so offset it into viewport
        // coordinates and hide it while it is outside the visible region.
        let cursor_point = grid.cursor.point;
        let cursor_viewport_line = cursor_point.line.0 + display_offset as i32;
        if !(0..num_lines as i32).contains(&cursor_viewport_line) {
            self.paint_scrollback_indicator(
                bounds,
                grid.total_lines(),
                num_lines,
                display_offset,
        ...
        let cursor_y = origin.y + self.cell_height * cursor_viewport_line as f32;
    ...
    fn paint_scrollback_indicator(
        ...
        let history_lines = total_lines - screen_lines;
        let track_height = bounds.size.height - px(8.0);
        let thumb_height = (track_height * screen_lines as f32 / total_lines as f32).max(px(18.0));
        let travel = (track_height - thumb_height).max(px(0.0));
        let from_top = history_lines.saturating_sub(display_offset) as f32 / history_lines as f32;
        let indicator = Bounds {
            origin: Point {
                x: bounds.origin.x + bounds.size.width - px(4.0),
                y: bounds.origin.y + px(4.0) + travel * from_top,
            },
            size: Size {
                width: px(2.0),
                height: thumb_height,
