...
/// Prepare clipboard text for terminal input.
...
pub fn keystroke_to_bytes(keystroke: &Keystroke, mode: TermMode) -> Option<Vec<u8>> {
    ...
    if key.len() == 1 {
        let ch = key.chars().next().unwrap();
        if ch.is_ascii() && !keystroke.modifiers.control && !keystroke.modifiers.platform {
...
mod tests {
    ...
    #[test]
    fn plain_paste_is_unchanged() {
        assert_eq!(paste_bytes("hello\n", TermMode::empty()), b"hello\n");
    }

    #[test]
    fn platform_shortcuts_do_not_leak_printable_input() {
        let command_c = Keystroke::parse("cmd-c").unwrap();
        assert_eq!(keystroke_to_bytes(&command_c, TermMode::empty()), None);
    }

    #[test]
    fn bracketed_paste_wraps_text_and_strips_escape_bytes() {
        assert_eq!(
            paste_bytes("hello\x1bworld", TermMode::BRACKETED_PASTE),
