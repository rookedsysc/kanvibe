...
use crate::input::{keystroke_to_bytes, paste_bytes};
use crate::mouse::{
    accumulate_scroll_lines, encode_modifiers, mouse_button_report, mouse_motion_report,
    pixel_to_cell, scroll_report,
};
...
pub struct TerminalView {
    ...
    /// Last measured grid geometry, shared with the canvas paint callback.
    render_metrics: Arc<parking_lot::Mutex<RenderMetrics>>,

    /// Fractional trackpad rows retained across precise scroll events.
...
impl TerminalView {
        ...
        R: Read + Send + 'static,
    {
        ...
        let render_metrics = Arc::new(parking_lot::Mutex::new(RenderMetrics {
            origin: point(config.padding.left, config.padding.top),
    ...
    fn on_key_down(&mut self, event: &KeyDownEvent, _window: &mut Window, cx: &mut Context<Self>) {
        ...
        if event.keystroke.modifiers.platform && event.keystroke.key.eq_ignore_ascii_case("v") {
            if let Some(text) = cx.read_from_clipboard().and_then(|item| item.text()) {
                let bytes = paste_bytes(&text, self.state.mode());
    ...
    ///
    /// Reports application mouse input, or starts a local text selection.
    fn on_mouse_down(
        &mut self,
        ...
        cx.stop_propagation();
        let Some((point, side)) = self.mouse_point(event.position) else {
            return;
        };
    ...
    ///
    /// Reports the release to mouse-aware terminal applications.
    fn on_mouse_up(&mut self, event: &MouseUpEvent, _window: &mut Window, cx: &mut Context<Self>) {
        let Some((point, _)) = self.mouse_point(event.position) else {
            return;
        };
        if !event.modifiers.shift
            && let Some(bytes) = mouse_button_report(
    ...
    ///
    /// Updates either application drag tracking or the local selection.
    fn on_mouse_move(
        &mut self,
        ...
        let mode = self.state.mode();
        if !event.modifiers.shift
            && let Some(bytes) = mouse_motion_report(
    ...
    ///
    /// Scroll locally, or report the wheel to an alternate-screen application.
    fn on_scroll(
        &mut self,
        ...
        let metrics = *self.render_metrics.lock();
        if event.delta.pixel_delta(metrics.cell_height).y == px(0.0) {
            return;
        }
        cx.stop_propagation();
        let lines = accumulate_scroll_lines(
            event.delta.pixel_delta(metrics.cell_height).y,
            metrics.cell_height,
        ...
        let mode = self.state.mode();
        if !event.modifiers.shift
            && let Some((point, _)) = self.mouse_point(event.position)
            && let Some(bytes) = scroll_report(
                lines,
                point,
                encode_modifiers(
                    event.modifiers.shift,
    ...
    fn mouse_point(&self, position: Point<Pixels>) -> Option<(AlacPoint, Side)> {
        let metrics = *self.render_metrics.lock();
        if metrics.cell_width <= px(0.0) || metrics.cell_height <= px(0.0) {
            return None;
        }

        let viewport_point = pixel_to_cell(
            position,
            metrics.origin,
            metrics.cell_width,
            metrics.cell_height,
        );
        let (columns, screen_lines, display_offset) = self.state.with_term(|term| {
            (
                term.columns(),
                term.screen_lines(),
        ...
        let column = viewport_point.column.0.min(columns.saturating_sub(1));
        let viewport_line = viewport_point
            .line
            .0
            .clamp(0, screen_lines.saturating_sub(1) as i32);
        let point = AlacPoint::new(Line(viewport_line - display_offset as i32), Column(column));

        let relative_x: f32 = (position.x - metrics.origin.x).into();
        let cell_width: f32 = metrics.cell_width.into();
        let within_cell = relative_x.max(0.0) % cell_width;
        let side = if within_cell < cell_width / 2.0 {
            Side::Left
        } else {
            Side::Right
        };
    ...
    fn write_user_input(&mut self, bytes: &[u8]) {
        self.state.with_term_mut(|term| {
...
impl Render for TerminalView {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let render_metrics = self.render_metrics.clone();
            ...
            .child(
                canvas(
                    ...
                    move |bounds, _, window, cx| {
                        ...
                        *render_metrics.lock() = RenderMetrics {
                            origin: point(
