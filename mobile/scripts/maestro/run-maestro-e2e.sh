...
if [[ "${PLATFORM}" == "android" ]] && command -v adb >/dev/null 2>&1; then
  ...
  # 화면 키보드가 떠 있으면 접근성 트리가 그만큼 커지고, 텍스트를 넣은 직후 기기 쪽 Maestro 드라이버가
  # 끊겨(UNAVAILABLE) 흐름이 멈춘다. 입력은 드라이버가 직접 넣으므로 IME 없이도 된다.
  adb shell ime disable com.google.android.inputmethod.latin/com.android.inputmethod.latin.LatinIME || true
fi
...
if [[ "${RECORD_VIDEO_ENABLED}" == true ]]; then
  ...
  # Maestro 2.10은 startRecording에 출력 폴더 밖 절대경로를 주면 흐름을 시작하기도 전에 거절한다.
  # 이름만 넘기고 실행이 끝난 뒤 아래에서 test-output-dir에 떨어진 파일을 원래 자리로 옮긴다.
